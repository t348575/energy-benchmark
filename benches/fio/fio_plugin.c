...
static void
set_feature_cb(void *ctx, const struct spdk_nvme_cpl *cpl)
{
    uint8_t fid = (uint8_t)( (uintptr_t)ctx );
    if (spdk_nvme_cpl_is_error(cpl)) {
		printf("SET FEATURE 0x%x failed (status=0x%x)\n", fid, cpl->status.sc);
    } else {
		printf("SET FEATURE 0x%x succeeded\n", fid);
    }
    g_done = true;
}
...
spdk_fio_setup(struct thread_data *td)
{
	...
	for_each_file(td, f, i) {
		...
		if (fio_ctrlr) {
			...
			uint8_t feature_id = 2;
			static void * const p_feature = (void*)(uintptr_t)2;
			fio_ctrlr = get_fio_ctrlr(&trid);
			printf("ctrlr: %p\n", fio_ctrlr);
			printf("spdk_nvme_ctrlr_cmd_set_feature\n");
			struct spdk_fio_options *fio_options = td->eo;
			spdk_nvme_ctrlr_cmd_set_feature(
				fio_ctrlr->ctrlr,
				feature_id,
				fio_options->power_state,
				0,
				NULL, 0,
				set_feature_cb,
				p_feature
			);
			printf("SET FEATURE 0x%x\n", 2);
			while (!g_done) {
				spdk_nvme_ctrlr_process_admin_completions(fio_ctrlr->ctrlr);
			}
		}
...
static struct fio_option options[] = {
	...
	},
	{
		...
		.lname		= "Power State",
		.type		= FIO_OPT_INT,
		.off1		= offsetof(struct spdk_fio_options, power_state),
		.def		= "0",
		.help		= "Power state value",
		.category	= FIO_OPT_C_ENGINE,
		.group		= FIO_OPT_G_INVALID,
	},
	{
