...
def plotter(name, bw_log, left_axis, filepath, label_left, offset=0,):
    fig, ax1 = plt.subplots(figsize=(12, 5))
    ax1.plot(bw_log["time"] + offset, bw_log["bw"], color=common.colors[0], label="Throughput (MiB/s)")
    ...
    fig.legend()
    plt.title(f"Throughput & SSD Power - APST")
    plt.tight_layout()
...
def gen_plots(plot_dir, results_dir, name, offset=0, trim=0):
    ...
    diskstat = None
    if os.path.exists(os.path.join(results_dir, "diskstat.csv")):
    ...
    if diskstat is not None:
        diskstat = common.fill_clean(diskstat, trim=trim_from_end, offset=offset, fillmode="spread", fillmodespread=10)
    ...
    if diskstat is not None:
        plotter(name, bw_log, [diskstat["time"], diskstat["total_smoothed"]], os.path.join(plot_dir, f"{name}-bw-vs-diskstat.pdf"), "Iostat throughput (MiB/s)", offset=ramp_time)
        plotter(name, bw_log, [diskstat["time"], diskstat["read_smoothed"]], os.path.join(plot_dir, f"{name}-bw-vs-read-diskstat.pdf"), "Iostat throughput (MiB/s)", offset=ramp_time)
        plotter(name, bw_log, [diskstat["time"], diskstat["write_smoothed"]], os.path.join(plot_dir, f"{name}-bw-vs-write-diskstat.pdf"), "Iostat throughput (MiB/s)", offset=ramp_time)
    plotter(name, bw_log, [ps3["time"], ps3["Total"]], os.path.join(plot_dir, f"{name}-bw-vs-ssd-power.pdf"), "SSD power (Watts)", offset=ramp_time)
    plotter(name, bw_log, [rapl["time"], rapl["total_smoothed"]], os.path.join(plot_dir, f"{name}-bw-vs-cpu-power.pdf"), "CPU power (Watts)", offset=ramp_time)
    plotter(name, bw_log, [sysinfo["time"], sysinfo["average_freq_node0"]], os.path.join(plot_dir, f"{name}-bw-vs-cpu-freq-0.pdf"), label_left="CPU frequency (MHz)", offset=ramp_time)
    plotter(name, bw_log, [sysinfo["time"], sysinfo["average_load_node0"]], os.path.join(plot_dir, f"{name}-bw-vs-cpu-load-0.pdf"), label_left="CPU load", offset=ramp_time)
    plotter(name, bw_log, [sysinfo["time"], sysinfo["average_freq_node1"]], os.path.join(plot_dir, f"{name}-bw-vs-cpu-freq-1.pdf"), label_left="CPU frequency (MHz)", offset=ramp_time)
    plotter(name, bw_log, [sysinfo["time"], sysinfo["average_load_node1"]], os.path.join(plot_dir, f"{name}-bw-vs-cpu-load-1.pdf"), label_left="CPU load", offset=ramp_time)

    ps3_mean = ps3["Total"].mean().item()
    rapl_mean = rapl["Total"].mean().item()
